        ...
        stage('k8s deploy') {
            steps {
                sshagent(['ajesh2217']) {
                    ...
                    sh "scp -o StrictHostKeyChecking=no deployment-webapp.yml service-webapp-np.yml ubuntu@54.168.204.100:/home/ubuntu/"
                    script{
                        try{
                            sh "ssh ubuntu@54.168.204.100 sudo kubectl apply -f ."
                        }
                        catch(error){
                            sh "ssh ubuntu@54.168.204.100 sudo kubectl create -f ."
                        }
                    }
                }
            }
